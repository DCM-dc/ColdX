...
DSH 负责模型连接、会话、权限、工具运行和插件生命周期。ColdX 维护自己的请求调度、增量协议解析、运行诊断、工作区界面与生成式交互，并保留原生数据和插件兼容性。

## 0.2.1 小绒 · 毛球伙伴

侧栏里的紫色毛球会眨眼、看向鼠标，点一下会轻轻弹起，给你一句短回应。它会跟随当前会话的忙碌、等待和错误状态；全部在本地运行，不消耗模型 Token。支持深浅色、键盘和减少动态效果，可在通用设置中关闭。详见 [小绒的交互与验证](docs/companion.md)。

## 0.2.0 执行层与工作区

- 新的请求调度限制并发、取消排队，并为根任务与子任务公平分配运行名额。
- 流式协议改为增量扫描，避免长文本重复扫描；上下文统计复用不可变子树，保持提供方请求原样。
- 紧凑任务首页、统一中性色界面、中央对话与右侧工作结果；保留加号、模式、模型控件、文件与电脑功能。
- 可展开的“运行详情”显示真实请求、耗时与最终工具结果，字符统计和全局并发明确区分。

架构、可复现实测和验证范围见 [0.2.0 技术记录](docs/kernel-workspace-2026-09-19.md)。1 MiB 合成流的本地协议解析中位数从约 13 秒降至 35 毫秒；这是解析器测试，不代表模型推理速度或整机任务质量。

## Windows 安装包

当前桌面版本为 **0.2.1 / Windows x64**。发行版本和安装附件见 [ColdX 0.2.1](https://github.com/DCM-dc/ColdX/releases/tag/v0.2.1)，安装包名称为 `ColdX-0.2.1-win-x64.exe`。
